...
- `osxcross-arm64`/`osxcross-x64`/`osxcross-universal` presets that
  ...
  live in the preset.
- `.gitlab-ci.yml` running `build:clang-x64` and `build:gcc-x64` — automatic on
  `dev` and `main`, started by hand on a merge request and required to pass
  before it merges — and `.github/workflows/build.yml` covering `msvc` (`x64`,
  `x86`, `arm64`) and `macos` (`arm64`, `universal`) on GitHub's free hosted
  Windows and macOS runners. Linux `arm64` is build-only (cross-compiled,
  cannot execute on an x64 runner); `msvc-arm64-2022` configures, builds, and
  tests like the other Windows jobs, on GitHub's native `windows-11-arm`
  runner. 32-bit (`x86`) jobs configure, build, and
  ...
  `.github/workflows/build.yml` also carries the `clang`/`gcc` Linux
  matrix (`x64`, `x86`, cross `arm64`), and adds a `mingw-w64`
  ...
  GitHub build status.
- A one-way mirror of every branch and tag to
  `https://github.com/ssoft-hub/scl-kit`, carried by the project's GitLab push
  mirror, plus a `close-mirror-prs.yml` GitHub workflow that closes any PR
  opened there with a pointer back to GitLab.
