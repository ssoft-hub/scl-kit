#!/bin/sh
# Build the unified Sphinx docs site (narrative + versioned API per module).
# Must be run from the scl-kit root directory.
#
# Usage: sh script/sphinx/build.sh [output_dir]
#   output_dir — defaults to doc/sphinx/
#
# Output structure:
#   <out>/            — top-level narrative docs
#   <out>/_pagefind/  — Pagefind search index
#   <out>/utility/    — utility version listing + versions.json
#   <out>/utility/<ver>/  — Sphinx API docs for utility@<ver>
#   <out>/feature/<ver>/  — Sphinx API docs for feature@<ver>
